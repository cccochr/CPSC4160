...
void HUD::draw() const {
  if(visible || initVis){
    IOmod::getInstance().writeText("Move around with WASD   Hit 'space to jump", position[0], position[1]);
    //IOmod::getInstance().writeText("Hit 'space' to jump", position[0], position[6]);
